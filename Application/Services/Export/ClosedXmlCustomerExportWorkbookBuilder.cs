...
public sealed class ClosedXmlCustomerExportWorkbookBuilder : ICustomerExportWorkbookBuilder
{
    ...
    private sealed class CustomerExportWorkbook(
        IReadOnlyList<CustomerExportColumn> columns,
        DateTime exportedAt,
        bool arabic)
        : ICustomerExportWorkbook
    {
        ...
        public void AddSheet(CustomerExportSheet sheet)
        {
            ...
            {
                worksheet.Cell(row, 1).Value = CustomerExportLabels.NoCustomersInArea(arabic);
            ...
            if (HasMoneyColumns())
                WriteTotalRow(
                    worksheet,
                    row,
                    CustomerExportLabels.AreaTotal(arabic),
        ...
        public void AddFlatSheet(string sheetName, IReadOnlyList<CustomerExportRow> rows)
        {
            ...
            worksheet.Cell(1, 1).Value = CustomerExportLabels.StackedTitle(arabic);
            worksheet.Cell(1, 1).Style.Font.Bold = true;
            worksheet.Cell(1, 1).Style.Font.FontSize = 14;
            worksheet.Cell(2, 1).Value = CustomerExportLabels.Customers(arabic);
            worksheet.Cell(2, 2).Value = rows.Count;
            worksheet.Cell(3, 1).Value = CustomerExportLabels.ExportedAt(arabic);
            worksheet.Cell(3, 2).Value = exportedAt;
            ...
            foreach (var customer in rows)
            {
                for (var i = 0; i < columns.Count; i++)
                    WriteCell(worksheet.Cell(row, i + 1), columns[i], customer);

                row++;
            }

            if (rows.Count > 0 && HasMoneyColumns())
                WriteTotalRow(worksheet, row, CustomerExportLabels.Total(arabic), rows);

            if (rows.Count > 0)
            {
                worksheet.Range(headerRow, 1, row - 1, columns.Count).SetAutoFilter();
        ...
        public void AddStructureSheet(AreaStructureSheet sheet)
        {
            ...
            worksheet.Cell(1, 1).Value = CustomerExportLabels.Area(arabic);
            worksheet.Cell(1, 2).Value = sheet.AreaName;
            worksheet.Cell(2, 1).Value = CustomerExportLabels.Boxes(arabic);
            ...
            {
                worksheet.Cell(row, 1).Value = CustomerExportLabels.NoBoxesInArea(arabic);
                worksheet.Cell(row, 1).Style.Font.Italic = true;
            ...
            var headers = new[]
            {
                CustomerExportLabels.StructureBox(arabic),
                CustomerExportLabels.StructureLocationNote(arabic),
                CustomerExportLabels.StructureNotes(arabic),
                CustomerExportLabels.StructureCustomers(arabic)
            };
            ...
            worksheet.Cell(row, 1).Value = CustomerExportLabels.Total(arabic);
        ...
        public void AddBoxSheet(CustomerExportBoxSheet sheet)
        {
            ...
            {
                worksheet.Cell(row, 1).Value = CustomerExportLabels.NoCustomersInBox(arabic);
                worksheet.Cell(row, 1).Style.Font.Italic = true;
                AdjustColumns(worksheet);
                return;
            }
            ...
            if (sheet.Plans.Count > 1 && HasMoneyColumns())
                WriteTotalRow(
                    worksheet,
                    row,
                    CustomerExportLabels.BoxTotal(arabic),
        ...
        public void AddBoxStructureSheet(BoxStructureSheet sheet)
        {
            ...
            worksheet.Cell(1, 1).Value = CustomerExportLabels.Area(arabic);
            worksheet.Cell(1, 2).Value = sheet.AreaName;
            worksheet.Cell(2, 1).Value = CustomerExportLabels.Box(arabic);
            worksheet.Cell(2, 2).Value = sheet.Box.Name;
            worksheet.Cell(3, 1).Value = CustomerExportLabels.LocationNote(arabic);
            SetText(worksheet.Cell(3, 2), sheet.Box.LocationNote);
            worksheet.Cell(4, 1).Value = CustomerExportLabels.Notes(arabic);
            SetText(worksheet.Cell(4, 2), sheet.Box.Notes);
            worksheet.Cell(5, 1).Value = CustomerExportLabels.Customers(arabic);
            worksheet.Cell(5, 2).Value = sheet.Box.CustomerCount;
            worksheet.Cell(6, 1).Value = CustomerExportLabels.ExportedAt(arabic);
            worksheet.Cell(6, 2).Value = exportedAt;
        ...
        public byte[] ToBytes()
        {
            ...
            {
                var empty = AddLocalizedWorksheet(CustomerExportLabels.EmptySheetName(arabic));
                empty.Cell(1, 1).Value = CustomerExportLabels.NothingMatched(arabic);
                empty.Cell(2, 1).Value = CustomerExportLabels.ExportedAt(arabic);
                empty.Cell(2, 2).Value = exportedAt;
            ...
            int customerCount)
        {
            ...
            worksheet.Cell(3, 1).Value = CustomerExportLabels.Customers(arabic);
            ...
            int startRow)
        {
            ...
            planCell.Value = CustomerExportLabels.Plan(planGroup.Plan, arabic);
            planCell.Style.Font.Bold = true;
            planCell.Style.Font.Italic = true;
            worksheet.Range(row, 1, row, columns.Count).Style.Fill.BackgroundColor =
            ...
            if (HasMoneyColumns())
            {
                WriteTotalRow(worksheet, row, CustomerExportLabels.PlanSubtotal(planGroup.Plan, arabic),
                    planGroup.Customers);
                row++;
            }
            ...
            int startRow)
        {
            ...
            {
                WriteTotalRow(worksheet, row, CustomerExportLabels.NamedTotal(group.BoxName, arabic),
                    group.AllCustomers);
                row++;
            }
        ...
        private void WriteColumnHeaders(IXLWorksheet worksheet, int row)
        {
            ...
            {
                var headerCell = worksheet.Cell(row, i + 1);
                headerCell.Value = CustomerExportLabels.Header(columns[i], arabic);
                headerCell.Style.Font.Bold = true;
                headerCell.Style.Fill.BackgroundColor = XLColor.FromHtml(HeaderFill);
                headerCell.Style.Border.BottomBorder = XLBorderStyleValues.Thin;
            }
        ...
        {
            var rows = customers as IReadOnlyList<CustomerExportRow> ?? [.. customers];
            ...
            var range = worksheet.Range(row, 1, row, Math.Max(columns.Count, LabelColumn()));
        ...
        private bool HasMoneyColumns() => columns.Any(CustomerExportColumns.IsMoney);

        private int LabelColumn()
        {
            for (var i = 0; i < columns.Count; i++)
                if (!CustomerExportColumns.IsMoney(columns[i]))
                    return i + 1;
        ...
        private void WriteCell(IXLCell cell, CustomerExportColumn column, CustomerExportRow row)
        {
            switch (column)
            {
                ...
                case CustomerExportColumn.CustomerType:
                    SetText(cell, CustomerExportLabels.CustomerType(row.CustomerType, arabic));
                    break;
                ...
                case CustomerExportColumn.CustomerStatus:
                    SetText(cell, CustomerExportLabels.CustomerStatus(row.CustomerStatus, arabic));
                    break;
                case CustomerExportColumn.CustomerRelation:
                    SetText(cell, CustomerExportLabels.CustomerRelation(row.CustomerRelation, arabic));
                    break;
        ...
        private void AdjustColumns(IXLWorksheet worksheet)
            => worksheet.Columns(1, Math.Max(columns.Count + 1, 2)).AdjustToContents();
        ...
        private string UniqueSheetName(string name)
        {
            ...
            if (string.IsNullOrWhiteSpace(cleaned))
                cleaned = CustomerExportLabels.EmptySheetName(arabic);
